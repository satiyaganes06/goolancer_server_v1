    ...
    public function uploadImage(Request $request)
    {
        try {
            ...
            if($request->input('whr') == 'UserProfileImage'){
                $path = $file->store('uploads/images/UserProfileImage');
            }else if($request->input('whr') == 'PostImage'){
                $path = $file->store('uploads/images/PostImage');
            }else if($request->input('whr') == 'CertificateDocument'){
                $path = $file->store('uploads/images/CertificateDocument');
            }else if($request->input('whr') == 'ServiceImage'){
                $path = $file->store('uploads/images/ServiceImage');
            }else if($request->input('whr') == 'ServiceDocument'){
                $path = $file->store('uploads/files/ServiceDocument');
            }else if($request->input('whr') == 'ServiceRequestImage'){
                $path = $file->store('uploads/images/ServiceRequestImage');
            }else if($request->input('whr') == 'PaymentReceipt'){
                $path = $file->store('uploads/files/PaymentReceipt');
            }else if($request->input('whr') == 'JobResultFile'){
                $path = $file->store('uploads/files/JobResultFile');
            }else{
